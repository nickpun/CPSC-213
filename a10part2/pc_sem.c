...
uthread_sem_t to_produce;
uthread_sem_t to_consume;
uthread_sem_t lock;
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_sem_wait(to_consume);
    uthread_sem_wait(lock);
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_sem_signal(lock);
    uthread_sem_signal(to_produce);
  }
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  // start the threads
  to_produce = uthread_sem_create(MAX_ITEMS);
  to_consume = uthread_sem_create(0);
